...
本仓库当前的 `config`、`session`、`eventbus` 模块真实 E2E 默认直接在宿主机运行，不使用 Docker。
...
5. 用 `pytest` 黑盒调用 `validate` / `run` 命令和 harness 子命令。
...
```bash
python3 -m pytest e2e/config e2e/session e2e/eventbus -v
```
